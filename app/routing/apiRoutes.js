...
module.exports = function(app) {
  ...
  app.post("/api/friends", function(req, res) {
    ...
    for (var i = 0; i < friends.length; i++) {
      ...
      if (totalDifference <= winningMatach.friendDifference) {
        winningMatach.name = currentFriend.name;
        winningMatach.photo = currentFriend.photo;
        winningMatach.friendDifference = totalDifference;
      }
    }

    friends.push(userData);
    res.json(winningMatach);
  });
};
